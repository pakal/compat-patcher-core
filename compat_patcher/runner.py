    ...
    def _apply_selected_fixers(self, fixers):
        ...
        for fixer in fixers:
            if fixer['fixer_id'] not in self._all_applied_fixers:
                self._patching_utilities.emit_log("Compat fixer '{}-{}' is getting applied".format(
                ...
                try:
                    ...
                    self._patching_utilities.emit_log("Compat fixer '{}-{}' was actually not applied, reason: {}".format(
                        fixer["fixer_family"], fixer['fixer_id'], e), level="WARNING"
                    )
            else:
                self._patching_utilities.emit_log("Compat fixer '{}' was already applied".
